...
int main(int argc, char *argv[]) {
  ...
  int rr = 0;
    ...
    if (rr == READ) { // 랜덤 숫자가 읽기 요청이라면
      SendMsg.type = READ_REQ; // 요청 타입을 읽기로 선언한다.
      strcpy(SendMsg.value, ""); 
      pkt_size = sizeof(struct KVS) - VALUE_SIZE; // 읽기 요청은 Value를 보낼 필요 없으므로 패킷 크기에서 VALUE_SIZE만큼 제외한다.  
    }
    else { // 랜덤 숫자가 쓰기 요청이라면
    ...
    sendto(sock, &SendMsg, pkt_size, 0, (struct sockaddr *)&srv_addr, sizeof(srv_addr)); // 생성한 메시지를 primary 서버로 송신한다.

    /* 수신 파트 */
    struct KVS RecvMsg={0,}; // 수신용으로 쓸 메시지 구조체 생성 및 초기화
    n = recvfrom(sock, &RecvMsg, sizeof(RecvMsg), 0, (struct sockaddr *)&src_addr, &src_addr_len); // primary 서버로부터 답장을 수신한다.
