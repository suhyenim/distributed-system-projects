...
int main(int argc, char *argv[]) {
  ...
  socklen_t src_addr_len = sizeof(src_addr);
	...
	while (!quit) {
		// sock 소켓으로부터 수신받은 패킷 데이터를 읽은 후, RecvMsg 구조체 변수에 저장한다. 클라이언트 관련 정보는 src_addr와 src_addr_len에 저장한다.
		n = recvfrom(sock, &RecvMsg, sizeof(RecvMsg), MSG_DONTWAIT, (struct sockaddr *)&src_addr, &src_addr_len);
		if (n > 0) { // 만약 수신받은 데이트가 0바이트를 초과한다면
			RecvMsg.type = READ_REP;  // 메시지 타입을 reply 변경해준다.
			strcpy(RecvMsg.value,"DDDCCCCBBBBAAAA"); // 답장으로 돌려보낼 value값인  DDDCCCCBBBBAAAA를 value 필드에 복사해준다.
			// sock 소켓을 통해 RecvMsg 구조체 변수에 들어있는 데이터를 RecvMsg 구조체 크기만큼 src_addr에 저장된 목적지(즉, 클라이언트)로 보낸다.
			sendto(sock, &RecvMsg, sizeof(RecvMsg), 0, (struct sockaddr *)&src_addr, sizeof(src_addr));
		}
	}
