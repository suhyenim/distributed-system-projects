...
int main(int argc, char *argv[]) {
  ...
  size_t pkt_size = 0;
    ...
    // 요청을 모든 서버에게 보낸다.
    for(int i = 0; i < 3; i++){
      srv_addr.sin_port = htons(SERVER_PORT + i);
    ...
    while(1){
      ...
      if (n > 0) { // 만약 송신한 데이터가 0바이트를 초과한다면 (즉, 1바이트라도 수신했다면)
        strcpy(value[msg_cnt++], RecvMsg.value);
        ...
        if (msg_cnt == 3){
          ...
          // value가 같은 서버가 있는지 확인한다.
          if(!strcmp(value[0], value[1])) printf("True\n\n");
          else if(!strcmp(value[1], value[2])) printf("True\n\n");
          else if(!strcmp(value[0], value[2])) printf("True\n\n");
          else printf("False\n\n");
