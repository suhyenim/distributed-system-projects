...
int main(int argc, char *argv[]) {
  ...
  int srv_msg[NUM_SRV] = {0};
    ...
    int port_num1 = rand() % NUM_SRV;
    int port_num2 = rand() % NUM_SRV;

    while(port_num1 == port_num2) port_num2 = rand() % NUM_SRV;

    if( srv_msg[port_num1] <= srv_msg[port_num2] ){
      srv_addr.sin_port = htons(SERVER_PORT + port_num1);
      srv_msg[port_num1]++;
    }
    else {
      srv_addr.sin_port = htons(SERVER_PORT + port_num2);
      srv_msg[port_num2]++;
    }
