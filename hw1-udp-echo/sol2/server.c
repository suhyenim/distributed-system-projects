...
int main(int argc, char *argv[]) {
  ...
  socklen_t src_addr_len = sizeof(src_addr);
	...
	while (!quit) {
		...
		n = recvfrom(sock, &RecvMsg, sizeof(RecvMsg), 0, (struct sockaddr *)&src_addr, &src_addr_len); // 클라이언트한테 메시지를 수신한다.
		if (n > 0) { // 만약 수신한 메시지의 데이터가 0바이트를 초과한다면 
			RecvMsg.type = READ_REP; // 메시지 타입을 read reply로 변경하고
			strncpy(RecvMsg.value, "DDDCCCCBBBBAAAA", VALUE_SIZE - 1); // value필드에 "DDDCCCCBBBBAAAA"를 넣는다.
			RecvMsg.value[VALUE_SIZE - 1] = '\0';  // 명시적으로 널 터미네이터 추가
			sendto(sock, &RecvMsg, sizeof(RecvMsg), 0, (struct sockaddr *)&src_addr, sizeof(src_addr)); // 수정한 메시지를 클라이언트로 송신한다.
		}
	}
