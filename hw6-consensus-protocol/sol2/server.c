...
void init_kvs(){
  for(int i =0;i<DATASET_SIZE;i++){
		...
		if(SERVER_PORT != 5003){ // 5001번, 5002번 포트를 사용하는 서버의 kvstore의 value값 설정
			strcpy(kv[i], "DDDDCCCCBBBBAAA");
		} 
		else { // 5003번 포트를 사용하는 서버의 kvstore의 value값 설정
			strcpy(kv[i], "SSWUCSE");
		}
		//printf("%s\n",kv[i]);
	}
...
int main(int argc, char *argv[]) {
	...
	size_t pkt_size = 0;
	while (!quit) {
		strcpy(RecvMsg.value,""); // RecvMsg 구조체 변수의 value필드를 비워줌 (비워주지 않으면 읽기 요청을 받았을 때, 이전 쓰기 요청의 value필드 값이 남아있게 됨)
		n = recvfrom(sock, &RecvMsg, sizeof(RecvMsg), MSG_DONTWAIT, (struct sockaddr *)&src_addr, &src_addr_len);
		if (n > 0) {
			printf("Type: %s Key: %s Value: %s\n", get_type(RecvMsg), RecvMsg.key, RecvMsg.value); // 수신 내용 출력
			RecvMsg.type = READ_REP; // 읽기 응답으로 type 변경
			uint64_t index = hash64(RecvMsg.key) % DATASET_SIZE;
			strcpy(RecvMsg.value, kv[index]); // kvstore에서 해당 index에 대한 value값을 메시지의 value값으로 복사
			pkt_size = n + VALUE_SIZE; // 읽기 답장은 VALUE필드를 포함해서 가므로, 기존 수신한 바이트 수(즉 읽기 요청의 크기)에 value_size를 더한다.
			sendto(sock, &RecvMsg, pkt_size, 0, (struct sockaddr *)&src_addr, sizeof(src_addr)); // client로 답장 전송
		}
	}
